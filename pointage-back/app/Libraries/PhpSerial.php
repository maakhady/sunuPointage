 ...
 class PhpSerial
{
    ...
    public function deviceSet($device)
    {
        if (!file_exists($device)) {
            trigger_error("Le port série n'existe pas : " . $device, E_USER_WARNING);
            return false;
        }

        $this->_device = $device;
        $this->_dState = SERIAL_DEVICE_SET;
        return true; // Indiquer que la configuration a réussi
    }
    ...
    public function confParity($parity)
{
    ...
    $cmd = "stty -F " . escapeshellarg($this->_device) . " " . $args[$parity];

    $ret = $this->_exec($cmd, $out);
    if ($ret === 0) {
        trigger_error("Parity configuration successful: $cmd", E_USER_NOTICE);
        return true;
    }

    trigger_error("Unable to set parity: " . ($out[1] ?? 'Unknown error'), E_USER_WARNING);
    return false;
}
    ...
    public function _exec($cmd, &$out = null)
{
    ...
    trigger_error("Exécution de la commande : $cmd", E_USER_NOTICE);

    $desc = array(
        1 => array("pipe", "w"), // sortie standard
        2 => array("pipe", "w")  // sortie d'erreur
    );
    ...
    if (is_resource($proc)) {
        ...
        if ($retVal !== 0) {
            trigger_error("Erreur : Commande échouée ($cmd). Détails : $stderr", E_USER_WARNING);
            return -1;
        }

        $out = array($stdout, $stderr);
        return $retVal;
    }

    trigger_error("Échec de l'ouverture du processus pour la commande : $cmd", E_USER_WARNING);
    return -1;
}
